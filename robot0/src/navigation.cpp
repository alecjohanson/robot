...
const std::pair<double,double> IR6_POS(-0.06,-0.1);
const double R_Base = 0.21; 							// robot base
const double R_Wheel_Radius = 0.05;
const double R_Max_Speed = 0.5; // maximum speed in meters/s (does not make sense, motor controller is weird)
...
std::vector<double> GetObjVector(std::vector<double> IR) {
	...
	//std::cerr << i<<":: LENGTH: "<<GetVectorLength(IR_vectors[i]) << std::endl;
		if (GetVectorLength(IR_vectors[i]) < 0.25 && GetVectorLength(IR_vectors[i]) > 0.0 ) { 	
...
std::vector <double> FollowVector (std::vector<double> U) {
	...
	double w = sin(-theta_d)*U[0] +1.0/eps*cos(-theta_d)*U[1]; // angular velocity
...
void test(const robot0::Distance &msg){

  	Speed spd;//the motors like in fakemotors
  
  	// Getting the info from the sensors (absolute).
  	std::vector<double> IR_measurements (6,1000);
  	IR_measurements[0] = (msg.front_r+Sharp2Wheel_FrontRightD)*0.01; 
  	IR_measurements[1] = (msg.front+ Sharp2Edge_FrontD)*0.01; 
  	IR_measurements[2] = (msg.front_l+Sharp2Wheel_FrontLeftD)*0.01; 
  	IR_measurements[3] = (msg.rear_l+Sharp2Wheel_RearLeftD)*0.01; 
  	IR_measurements[4] = (msg.rear+ Sharp2Edge_RearD)*0.01; 
  	IR_measurements[5] = (msg.rear_r+Sharp2Wheel_RearRightD)*0.01; 
