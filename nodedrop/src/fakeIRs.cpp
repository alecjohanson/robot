...
int main(int argc, char **argv) {
	...
	bool on_node;
	while (ros::ok()) {
		//if ( last_state != state && debug == true) {std::cerr << "NEW STATE: " << state << " (LAST STATE: "<<last_state<<")"<<std::endl; last_state = state;}
		...
		switch (state) {
			...
			case 2:	// turn
				...
				if (current_node->getExplored(dir_robot2frame(r_right,dir)) == 1) {
					...
					dir = dir.rot90L().rot90L(); // turn 180
				}else { // backtrack
					// turn and head to last node
					if(BACKTRACKING == false) {
						BACKTRACKING = true;
						goal = backtrack(current_node);
						if(goal == NULL)  {std::cerr<< "MAP EXPLORED!" << std::endl;return 0;}
